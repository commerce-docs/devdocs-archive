...
The build checks if the [`config.local.php` file](https://devdocs.magento.com/guides/v2.1/cloud/live/sens-data-over.html) exists in the codebase and has information about scopes (for details see [`SCD_STRATEGY`](https://devdocs.magento.com/guides/v2.2/cloud/env/environment-vars_magento.html)). If so, static files are deployed during this phase, reducing the downtime in the deployment phase.
